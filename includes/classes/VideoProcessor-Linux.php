...
class VideoProcessor {
    ...
    public function generateThumbnails($filePath) {
		...
		$query1 = $this->con->prepare("SELECT id as lastid FROM videos order by uploaddate desc FETCH FIRST 1 ROWS ONLY");
		$query1->execute();
		$sqlData1 = $query1->fetch(PDO::FETCH_ASSOC);
		$videoId = intval($sqlData1["LASTID"]);
        ...
        for($num = 1; $num <= $numThumbnails; $num++) {
            ...
            $query = $this->con->prepare("INSERT INTO thumbnails(filePath, selected , videoId)
                                        VALUES(:filePath, :selected, :videoId)");
